...
public class SetDataUtils {
    private static SetDataUtils utils;
    ...
    public <T extends Object> T createData(T t) {
        try {
            //得到所有属性
            ...
            for (int i = 0; i < field.length; i++) {
                ...
                if (type.equals("class java.lang.Float") || type.equals("float")) {
                    float random = (float) (Math.random() * 10.0f + 1);
                    field[i].set(t, random);
                }
                if (type.equals("class java.lang.Double") || type.equals("double")) {
                    double random = (Math.random() * 10.003333333330 + 1);
                    field[i].set(t, random);
                }
                if (type.equals("class java.util.Date")) {
                    field[i].set(t, new Date());
                }
            }
    ...
    private String getRandomWord(int minLength, int maxLength) {
        ...
        int length = (maxLength - minLength > 0 ? rand.nextInt(maxLength - minLength) : 0) + minLength;
        for (int i = 0; i < length; i++) {
            string.append(StringBase.base.charAt(new Random().nextInt(StringBase.base.length())));
        }
